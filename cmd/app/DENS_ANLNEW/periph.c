...
BOOL bAnlSgnlsUpdated = FALSE;
xdata BYTE i2cwrbuf[2+64];
xdata BYTE debug_string[48];
...
void TD_Poll(void)
{
  ...
  if (bAnlSgnlsUpdated)
  {
    ...
    {
      Set_DC(i, valSignals.anlgSgnls[i]);
    }
    Update_DC();
    bAnlSgnlsUpdated = FALSE;
  }
}
...
BOOL DR_VendorCmnd(void)
{
  switch (SETUPDAT[1])
  {
    ...
    case 0xB1:
      ...
      {
        case 0xC0: // вывод
          *(PANL_CORRECTION)(EP0BUF) = corr_data;

          EP0BCH = 0; 
          EP0BCL = sizeof(corr_data);
        break;
        case 0x40: // ввод
          EP0BCH = 0; 
          EP0BCL = 0; 
          ...
          corr_data = *(PANL_CORRECTION)EP0BUF;
        
          i2cwrbuf[0] = 0;
          i2cwrbuf[1] = EEPROM_LOC; 
          memcpy(&i2cwrbuf[2], &corr_data.corr[0], sizeof(corr_data));
          EZUSB_WriteI2C(TAG_24LC256, 64+2, &i2cwrbuf[0]);
          EZUSB_WaitForEEPROMWrite(TAG_24LC256);
